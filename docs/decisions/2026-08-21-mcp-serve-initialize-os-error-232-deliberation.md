...
5. **Keep H3 modes separate.** H3-A transport/framing belongs to `056.011-T`,
   ...
   persistence); it must use the Rust-1.75-compatible rmcp dependency strategy
   selected and pinned by `056.029-T` without changing that strategy. Any
   source/API migration required by the selection is a named bounded follow-up,
   not implicit `056.011-T` scope. H3-B client capability belongs to
   `056.019-T`, the sole H3-B terminal, which consumes T0's
...
* **The rmcp 1.8.x exclusion rationale is corrected.** "1.8.x is excluded by
  ...
  by `056.029-T`, one of the five members of the PHASE 1.6 shipment `053-S`
  (`056.029-T` through `056.033-T`), which runs BEFORE `052-S`. `056.011-T`
  consumes the completed `053-S` compatibility evidence and runs its ordinary
  validation at the unchanged Rust/Cargo 1.75 floor without a separate MSRV
  entry gate.
