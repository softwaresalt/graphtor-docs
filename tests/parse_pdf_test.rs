...
/// public-domain PDF with two font sizes, generated by `scripts/gen_test_pdf.py`.
#[test]
fn parse_pdf_heading_aware_real_pdf() {
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/sample_heading.pdf");
    ...
    // At least one chunk must have a non-empty heading hierarchy — the
    // 18pt heading must have been detected and propagated.
    let has_heading = doc
        .chunks
        .iter()
        .any(|c| !c.heading_hierarchy.is_empty());
    assert!(
        has_heading,
        "at least one chunk must have a non-empty heading_hierarchy; \
         chunks: {:#?}",
        doc.chunks
            .iter()
