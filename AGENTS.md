...
graphtor-docs is a single-developer Rust MCP server project focused on local documentation cross-referencing. All processing is local-only — no data leaves the developer’s machine. The system acquires documentation from configured sources, chunks and embeds content using in-process ML inference, and serves semantic search queries via MCP tools over STDIO transport. Development follows conventional commits on feature branches with GitHub Actions CI (fmt → clippy → test → audit).
...
`#![forbid(unsafe_code)]` at crate root. All clippy warnings (including `clippy::pedantic`) are hard errors via `-D warnings`.
All errors must be propagated via `Result<T, E>`; no `.unwrap()` or `.expect()` in library code.
