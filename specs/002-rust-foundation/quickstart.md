...
```rust
use graphtor_core::SourceConfig;
use std::path::Path;
...
```rust
use graphtor_core::generate_chunk_id;
...
```rust
use graphtor_core::validate_path;
use std::path::Path;
...
// This would fail with GraphtorError::PathViolation:
...
```rust
use graphtor_core::{init_logging, LogVerbosity};
...
match some_operation() {
    ...
    Err(GraphtorError::Config { message, field: _ }) => {
