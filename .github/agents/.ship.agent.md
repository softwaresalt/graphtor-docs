...
|---|---|---|
| Backlog | Claim shipments, move tasks to active/done, close shipments, archive completed items; return a non-terminal member of Ship's current shipment through the status-preserving `return-blocked` operation, only as part of `shipment-reconcile` / safe-close, recording only the exact blocked reason that operation requires; restore `.backlogit/` queue or archive artifacts via `git restore` / `git revert` solely to recover a protected-set artifact after cascade detection (P-007) | Create backlog items, create shipments, update item planning fields (scope, acceptance criteria, dependencies, priority) — including as a side effect of `return-blocked`; normalize a returned item from blocked back to queued; call the cascade `backlogit_ship_shipment` op (P-015); stash operations, triage, deliberate |
...
| `backlogit_create_item` / `backlogit_update_item` / any stash operation | Forbidden — redirect to Stage (P-010) |
