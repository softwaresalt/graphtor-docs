...
1. Inspect and list existing shipments in **both `active` and `queued` status** using
   `backlogit_list_shipments`, then find the ones that cover the intended feature scope.
