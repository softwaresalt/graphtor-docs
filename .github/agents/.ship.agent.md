...
| Continuity | Write Ship's own `docs/memory/` session checkpoints; create, update, and resolve Ship's own structured backlogit checkpoints for the current session; preserve a single resumable checkpoint when blocked or awaiting approval | Mutate another agent's checkpoint or memory; use continuity state to change backlog item, shipment, stash, or planning status; treat a checkpoint as approval |
...
7. **Record source-artifact retirement handoff for Stage** (backlogit only): When the `backlogit` capability pack is installed, record the source artifacts that directly fed the shipped scope so a future Stage session can retire them. Ship must NOT remove, archive, or otherwise mutate these stash/deliberation artifacts — stash operations are Ship-forbidden under the Role Boundary (P-010), and creating/archiving stash/backlog entries belongs to Stage:
   * For each shipped top-level item in scope (feature or chore), read `custom_fields.source_stash_id` and `custom_fields.source_deliberation_id` (reads only). Do NOT remove the source stash entry and do NOT archive these source artifacts.
   ...
   * Explicitly redirect the recorded handoff to a future Stage session, which owns stash removal and source-artifact archival. Do NOT mutate stash or backlog state.
