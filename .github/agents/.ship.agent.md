...
|---|---|---|
| Backlog | Claim shipments, move tasks to active/done, close shipments, archive completed items; move the current shipment record through the `shipment-reconcile` safe-close status transition and archive that same shipment record as its own single artifact; return a non-terminal member of Ship's current shipment through the status-preserving `return-blocked` operation, only as part of `shipment-reconcile` / safe-close, recording only the exact blocked reason that operation requires; record the actual merge commit of Ship's current shipment or its member tasks as closure evidence via `backlogit_track_commit` (or the registry-equivalent `commit` field); restore `.backlogit/` queue or archive artifacts via `git restore` / `git revert` solely to recover a protected-set artifact after cascade detection (P-007) | Create backlog items, create shipments, update item planning fields (scope, acceptance criteria, dependencies, priority) — including as a side effect of `return-blocked` or of commit tracking; normalize a returned item from blocked back to queued; move, archive, or otherwise mutate any shipment record other than Ship's current one; call the cascade `backlogit_ship_shipment` op (P-015); stash operations, triage, deliberate |
...
| `backlogit_move_item` | Allowed — move members of Ship's current shipment through `active` / `done` execution status, and move that same current shipment record through the `shipment-reconcile` safe-close status transition as part of closure. Bounded to the exact shipment Ship claimed this session and its manifest members: no other shipment record, no item outside that manifest, and no status change beyond execution and safe-close |
| `backlogit_archive_item` | Allowed — single-artifact archival during `shipment-reconcile` safe-close: each manifest item of Ship's current shipment, and that same current shipment record itself, archived as its own single artifact and never through the destructive cascade op (P-015). Bounded to the exact current shipment: no other shipment record, no item outside its manifest, and no archival outside safe-close |
...
| `backlogit_track_commit` | Allowed — evidence only. Records the actual merge commit SHA (already confirmed on `origin/main`) of Ship's current shipment or one of its member tasks on that item, as part of closure/reconciliation, via `backlogit_track_commit` or the registry-equivalent `commit` field. It records commit evidence and nothing else: no scope, acceptance-criteria, dependency, or priority change, no status change, no fabricated or anticipated SHA, and no authority over any item outside Ship's current shipment |
...
| `backlogit_create_item` / `backlogit_update_item` / any stash operation | Forbidden — redirect to Stage (P-010). The single carve-out is the registry `track_commit` mapping's `commit` field, classified above as evidence only; it never licenses any other `update` field |
...
1. Inspect and list existing shipments in `queued` status using `backlogit_list_shipments`
   to find one that already covers the intended feature scope. If a suitable Stage-prepared
   shipment exists, record its ID and proceed exactly as the primary path — validate the
   manifest and item membership, run the Branch Creation Gate + Worktree Topology Gate
   (primary-path step 3a), claim it, and run the intake reconciliation check.
2. If no suitable shipment exists, **halt and redirect to Stage** so it can assemble a
   shipment through the full triage → deliberate → plan → review → harvest → shipment
   pipeline. Ship must not create the shipment, add items, or normalize item status itself.
...
After a shipment is selected and claimed via either path, the intake reconciliation check from
...
1. **Close the shipment** (when `true` is true):
   ...
   b. **Safe-close (single-artifact archival — NEVER the cascade op, P-015)**: Invoke the
      ...
      record itself as its own single artifact — moving that current shipment record through
      the same safe-close status transition and then archiving it, both classified allowances
      that reach Ship's current shipment record and no other shipment — all to
      `.backlogit/archive/`.
      ...
      * **Commit evidence (evidence only)**: as part of closure/reconciliation, record the
        confirmed `merge_commit_sha` on the current shipment and on its member tasks using
        `backlogit_track_commit` (registry `track_commit`; CLI mapping
        `backlogit update {id} --commit {sha}`), before each item is archived if the archive
        operation carries no commit parameter. Record only the actual merge commit already
        confirmed on `origin/main` by the Merge Confirmation Gate — never an anticipated,
        reconstructed, or branch-tip SHA. This grants no planning-field authority and no
        status authority (P-010), and applies to no item outside this shipment.
