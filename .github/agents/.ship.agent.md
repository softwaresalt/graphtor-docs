...
| `backlogit_track_commit` | Allowed — evidence only. Records the actual merge commit SHA of the delivered work (already confirmed on `origin/main`) for Ship's current shipment or one of its member tasks on that item, via `backlogit_track_commit` or the registry-equivalent `commit` field, recorded by `shipment-reconcile` safe-close **before** that item is archived. It records commit evidence and nothing else: no scope, acceptance-criteria, dependency, or priority change, no status change, no fabricated or anticipated SHA, no decision/closure-authority SHA substituted for the delivered-work merge SHA, and no authority over any item outside Ship's current shipment |
...
1. **Close the shipment** (when `true` is true):
   ...
   b. **Safe-close (single-artifact archival — NEVER the cascade op, P-015)**: Invoke the
      ...
      * **Commit evidence is recorded by the skill, before archival (evidence only)**: Ship
        does not record commit evidence after closure, and never after archival — once an
        item is archived its source record may no longer be mutable. Ship's only
        responsibility is to pass the correct `merge_commit_sha` into safe-close; the skill
        records it — via atomic archive commit metadata or `backlogit_track_commit`
        (registry `track_commit`; CLI mapping `backlogit update {id} --commit {sha}`) — on
        each live manifest item and on the shipment record **before** that artifact is
        archived, per its Safe-Close Mode
        steps 4 and 8. The SHA Ship passes MUST be the **actual delivered-work merge
