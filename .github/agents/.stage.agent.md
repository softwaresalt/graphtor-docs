...
1. **Check for an existing shipment in `queued` status** that already covers the
   harvested feature scope using `backlogit_get_queue`. If one already exists for this
...
3. **Scope guard (mandatory first step)**: Record the exact list of IDs returned by the
   ...
   for shipment assembly. `backlogit_update_item` MUST ONLY be called for items that
...
4. **Add remaining items in parent-first, dependency order** using `backlogit_update_item`:
...
5. **Verify the manifest** by reading back the shipment using `backlogit_get_item` and
...
2. When the `backlogit` capability pack is installed and the registry advertises checkpoint recovery operations, resolve any still-active checkpoints from the current session with `backlogit_resolve_checkpoint`. When the next action must survive a context-window shutdown, leave at most one final best-effort checkpoint written via `backlogit_save_checkpoint` with a clear `resume_hint`.
