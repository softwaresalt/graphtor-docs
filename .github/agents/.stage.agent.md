...
| `backlogit_create_shipment` / `backlogit_add_to_shipment` | Allowed — assemble the shipment manifest from `assembly_ids` only, never from `handoff_ids` and never from a `prerequisite_ids` entry (Step 5.5) |
...
1. **Scope guard (mandatory first step)**: Establish the canonical assembly scope through
   ...
   * `prerequisite_ids` — the external gates and terminal prerequisites the members depend on.
...
2. **Validate the ratified sets before use (Mode R only)**. Validate each set against its own
   ...
   **2a. Every ID in `member_ids` must satisfy all of:**
   ...
   e. has no unresolved dependency outside the declared sets, except a dependency on a
      satisfied `prerequisite_ids` entry, on an intentionally terminal prerequisite (a
      permanently blocked or superseded unit the ratification records as terminal), or on an
      explicit gate task named in `prerequisite_ids`.
...
3. **Define `assembly_ids`** as exactly one of: the fresh `harvest_ids` from Mode H, or the
   ...
   in `assembly_ids`. Never expand scope by searching the queue for unassigned items.

4. **Check for an existing shipment in `queued` status** that already covers the scope using
   `backlogit_list_shipments`. Reuse it ONLY when its current manifest is a subset of
   `assembly_ids` — the exact validated set — so that adding the remaining `assembly_ids`
...
5. **Create the shipment** (when no reusable shipment exists):
   ...
   * The covering feature ID used here must itself be a member of `assembly_ids`.
   * Record the resulting `shipment_id` as the session output token.
   * Broadcast `[STAGE] Created shipment: {shipment_id} — "{title}"`.

6. **Add remaining items in parent-first, dependency order** using `backlogit_add_to_shipment`:
   ...
   d. **Add-failure handling branches by mode — this is the fail-closed boundary:**
      * **Mode R (fail-closed; no partial shipment ever)**: any add failure, a member found
        ...
        shipment unpublished and un-handed-off, and re-run validation from step 2 once the
        cause is resolved. In Mode R, never publish a partial shipment.
...
shipment, and halt rather than publish a partial one.
