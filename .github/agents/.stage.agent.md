...
|---|---|
| `backlogit_create_shipment` / `backlogit_add_to_shipment` | Allowed — assemble the shipment manifest from `assembly_ids` only (Step 5.5) |
| `backlogit_move_item` | Allowed — apply a status Stage has explicitly ratified, and archive consumed stash entries (Step 5.6) |
| `backlogit_stash_archive` | Allowed — retire a consumed or handed-off stash entry while preserving traceability; the state-appropriate default for Ship's source-artifact retirement handoff |
...
| `backlogit_create_checkpoint` / `backlogit_resolve_checkpoint` | Allowed — Stage-owned continuity state only, per the Continuity row |
...
| `backlogit_claim_shipment` / `backlogit_ship_shipment` / shipment closure | Forbidden — redirect to Ship (P-010) |
...
1. **Scope guard (mandatory first step)**: Establish the canonical assembly scope through
   ...
   **Mode H — fresh harvest (default)**: Record the exact list of IDs returned by the
   ...
   emitted by this harvest MUST be excluded, even if they appear un-assigned and ready.
   ...
   **Mode R — ratified existing-scope handoff (recovery path)**: Use this mode only when the
   ...
   ratification artifact, or a Ship closure/memory handoff record. Record those named IDs
   verbatim as `handoff_ids`.
   ...
   * Never discover or expand the handoff set by scanning the queue for unassigned,
     ready-looking, or topically related items. `handoff_ids` is exactly the named set.

2. **Validate `handoff_ids` before use (Mode R only)**. Every ID must satisfy all of:
   ...
   e. has no unresolved dependency outside the declared handoff set, except a dependency on
      an intentionally terminal prerequisite (a permanently blocked or superseded unit the
      ratification records as terminal) or on an explicit gate task included in the set.
   Also reject the set when it names the same item twice (duplicate detection) or contains
   any ID the authorization did not name. On any unresolved failure, **halt fail-closed**:
   report the offending IDs and the failed condition, and assemble nothing. Partial
   assembly from an invalid handoff set is forbidden.

3. **Define `assembly_ids`** as exactly one of: the fresh `harvest_ids` from Mode H, or the
   validated `handoff_ids` from Mode R. Every remaining step in this section — shipment
...
6. **Add remaining items in parent-first, dependency order** using `backlogit_add_to_shipment`:
   ...
   d. If an item cannot be added (duplicate, already assigned to another shipment, or
      ...
      In Mode R this branch is an anomaly rather than routine — every member already passed
      step 2 — so record each discrepancy explicitly in the session summary.
...
7. **Verify the manifest** by reading back the shipment using `backlogit_get_shipment` and
   ...
   extras. Report any discrepancies.
...
blocked while any gate task named by the ratification is still open. For the 059-F handoff
specifically, successor-shipment assembly remains gated on 059.014-T sign-off — the path
exists now, but Stage must not assemble until that sign-off lands. Status normalization
already ratified for those units is a separate, completed act and does not satisfy this gate.

**Guardrail**: Do not assemble a shipment when `assembly_ids` is empty, when Mode H produced
no items or items with unresolved P-003 violations, or when Mode R validation failed or its
declared gate is still open. Halt and report before creating an empty shipment.
...
2. For each consumed stash entry:
   * When `backlogit` is the installed backlog tool and the registry advertises the stash
     ...
     approval. If the archive operation is unavailable, fall back to `backlogit_move_item` to
     move the entry from the stash to the archive.
