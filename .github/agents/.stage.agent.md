...
|---|---|---|
| Backlog | Create, update, and archive backlog items, stash entries, and shipment manifests; append audit comments; add and remove dependency edges and artifact links; apply Stage-ratified status normalization | Claim or close shipments on behalf of Ship; record commit evidence; delete items or stash entries |
...
| `backlogit_update_item` | Allowed — write Stage-owned planning fields on items Stage owns: title, description, scope, acceptance criteria, priority, execution posture, and provenance custom fields (`source_stash_id(s)`, `source_deliberation_id(s)`) |
...
| `backlogit_archive_item` | Allowed — archive a Stage-owned backlog item that was superseded or withdrawn before shipment. Never an item Ship's current shipment owns |
...
| `backlogit_stash_remove` | Forbidden by default — destructive deletion, deprecated in favour of `backlogit_stash_archive`; permitted only as an exceptional action behind its own explicit operator approval gate |
...
| `backlogit_claim_shipment` / `backlogit_ship_shipment` / `backlogit_return_blocked` / shipment closure | Forbidden — Ship-only claim, close, and merge-side operations; redirect to Ship (P-010) |
...
2. **Validate the ratified sets before use (Mode R only)**. Validate each set against its own
   rules. A member rule never applies to a prerequisite, and a prerequisite rule never
   licenses a member.

   **2a. Every ID in `member_ids` must satisfy all of:**
   ...
   c. is not already a member of another queued or active shipment;
   ...
   e. has **no dependency on any item outside `member_ids`** unless that dependency is named in
      `prerequisite_ids` and satisfies 2b at assembly time — including an explicit gate task,
      which counts only when it is named in `prerequisite_ids` and satisfied under 2b. Every
      ...
      dependency itself becomes satisfied under 2b. An external dependency the authorization
      never named is a halt, not a carve-out.

   **2b. Every ID in `prerequisite_ids` must satisfy all of:**
   a. is `done` or otherwise satisfied at assembly time — resolve it in the queue and in the
      ...
      condition 2a(a) to a prerequisite is itself an error;
...
3. **Define `assembly_ids`** as exactly one of: the fresh `harvest_ids` from Mode H, or the
...
4. **Check for an existing shipment in `queued` status** that already covers the scope using
   `backlogit_list_shipments`. `backlogit` has **no unpublished shipment state** — a `queued`
   shipment is already visible and claimable by Ship — so reuse branches by mode:
   * **Mode R (exact match only)**: reuse a queued shipment ONLY when its current manifest is
     **exactly equal** to `assembly_ids` — same count, same IDs, no extras and no missing
     member — because such a shipment already *is* the ratified set and needs no mutation.
     A subset, a superset, or any other partial overlap is NOT reusable in Mode R: halt,
     report the offending shipment ID and the exact difference, and hand off to the operator
     (or a later ratified Stage decision) — never add to it, never remove from it, never
     delete it, and never repurpose it. Stage holds no deletion authority, so a mismatched
     shipment is an operator decision, never a Stage cleanup.
   * **Mode H (subset reuse)**: reuse a queued shipment when its current manifest is a subset
     of `assembly_ids` — the exact validated set — so that adding the remaining `assembly_ids`
     in step 6 sweeps in no unrelated item. If its manifest holds any ID outside
     `assembly_ids`, do not reuse it: report the mismatch and halt instead of assembling into a
     shipment with unrelated scope.

5. **Create the shipment** (when no reusable shipment exists) — creation branches by mode:
   ...
   * **Registry capability gate (Mode R, evaluated before creating anything)**: confirm the
     ...
     single call (the backlogit registry maps `items` on `backlogit_create_shipment`; CLI
     `backlogit shipment create --title {title} --items {items}`). If it cannot create with the
     complete list atomically — the parameter is absent, the call rejects a multi-item list, or
     the registry only supports incremental population — **halt fail-closed**, create nothing,
     report the capability gap, and hand the assembly off to the operator. Degrading to
     feature-only-then-add is forbidden.
   ...
   * **No partial artifact on failure (Mode R)**: a failed create, or any validation failure
     ...
     artifact despite reporting failure, halt immediately, record it as a **P-010 /
     tool-contract failure** naming the stray shipment ID, and require operator or a subsequent
     ratified Stage recovery decision before anything else touches it. Stage must never delete
     or prune the stray artifact without explicit operator approval; deletion is forbidden for
     Stage and is not a recovery path.
...
6. **Add remaining items in parent-first, dependency order — Mode H only** using
   ...
   d. **Failure handling branches by mode — this is the fail-closed boundary for all of
      ...
      * **Mode R (fail-closed; no partial shipment ever)**: Mode R performs no incremental
        ...
        concurrently assigned to another shipment, any status drift since step 2, or any
        ...
        step 2 once the cause is resolved. In Mode R, never publish a partial shipment.
...
7. **Verify the manifest** by reading back the shipment using `backlogit_get_shipment` and
   confirming that its membership matches `assembly_ids` exactly: same count, same IDs, no
   extras, and no `prerequisite_ids` entry present.
   * **Mode R**: exact manifest equality is the publication gate. The `shipment_id` becomes
     the authoritative handoff token only after this check succeeds. Any discrepancy —
     missing member, extra ID, count mismatch, or a prerequisite that leaked into the
     manifest — is a halt under step 6.d, not a report-and-proceed.
...
* `member_ids` are exactly these 9 IDs — the covering feature plus eight tasks: `059-F`,
  `059.001-T`, `059.002-T`, `059.003-T`, `059.004-T`, `059.005-T`, `059.006-T`, `059.010-T`,
  `059.011-T`. They are the shipment members and become `assembly_ids`.
* `prerequisite_ids` are exactly `059.007-T` and `059.014-T`, and neither is a shipment member.
...
* `059.014-T` MUST NOT be added to the successor shipment. Once sign-off lands it moves to
  ...
  2a(a), and a `done`/archived item is not re-shipped. Treating it as a member would force
