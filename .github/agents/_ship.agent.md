...
|---|---|---|
| Backlog | Claim shipments, move tasks to active/done, close shipments, archive completed items; create a capture-only stash entry **solely for a P-021 C2 deferred-scope-expansion capture** (the C5 carve-out), which C2 makes a mandatory precondition for closing an out-of-scope finding | Create backlog items, create shipments, update item planning fields (scope, acceptance criteria); triage, prioritize/re-prioritize, re-classify, edit, harvest, or deliberate on stash entries; removal or archival of stash entries, including the post-merge Step 7 source-artifact retirement; creating a stash entry for any purpose other than a P-021 C2 capture, including the pre-merge Step 10 and post-merge Step 6 follow-up handoffs, which are handoff-only |
...
| `backlogit_stash` (capture-only creation) | Allowed **only** as the P-021 C2 deferred-scope-expansion capture: a single stash-entry creation carrying the C2 payload (the literal `DEFERRED SCOPE EXPANSION` token, the expansion statement, the C1 out-of-scope rationale, per-field source refs, the `requires deliberation` flag, kind and provisional priority). C2 makes this capture a **precondition** for closing an out-of-scope finding and C7 halts without it, so this grant is what makes a mandatory policy satisfiable. It confers no further stash authority: Ship never triages, prioritizes, re-classifies, edits, harvests, deliberates on, removes, or archives any stash entry — including the one it just created — and never creates a stash entry for any other purpose |
| `backlogit_create_item` / `backlogit_delete_item` / every other stash operation | Forbidden — redirect to Stage (P-010). Ship never creates a backlog item or shipment, and never deletes a mistakenly created or mismatched shipment or item; deletion is an operator decision, never a Ship cleanup. Beyond the single C2 capture row above, every stash verb is forbidden, including `backlogit_stash_archive` and `backlogit_stash_remove`: the pre-merge Step 10 and post-merge Step 6 follow-ups and the post-merge Step 7 source-artifact retirement are recorded as handoffs for Stage, never executed by Ship |
...
10. **Record follow-up handoff for Stage**: If the closure artifact, runtime-verification report, or local review readiness result identified follow-up tasks, record every follow-up for a future Stage session. Ship must NOT create, update, or append any stash or backlog artifact for these follow-ups — follow-up stash operations and backlog-item creation are Ship-forbidden under the Role Boundary (P-010); creating stash/backlog entries belongs to Stage. This handoff-only rule governs **follow-ups**; it does not reach the separate, mandatory P-021 C2 deferred-scope-expansion capture, which Ship performs itself under the C5 capture-only carve-out and which C2 makes a precondition for closing an out-of-scope finding:
...
6. **Record follow-up handoff for Stage**: If the post-merge closure artifact identified follow-up tasks (monitoring gaps, deferred scope, documentation debt, or any action not covered by the shipped work), record every follow-up for a future Stage session. Ship must NOT create, update, or append any stash or backlog artifact for these follow-ups — follow-up stash operations and backlog-item creation are Ship-forbidden under the Role Boundary (P-010); creating stash/backlog entries belongs to Stage. As at pre-merge Step 10, this handoff-only rule governs **follow-ups** and does not reach the mandatory P-021 C2 deferred-scope-expansion capture, which Ship performs itself under the C5 carve-out:
...
7. **Record source-artifact retirement handoff for Stage** (backlogit only): When the `backlogit` capability pack is installed, record the source artifacts that directly fed the shipped scope so a future Stage session can retire them. Ship must NOT remove, archive, or otherwise mutate these stash/deliberation artifacts — stash operations are Ship-forbidden under the Role Boundary (P-010), and creating/archiving stash/backlog entries belongs to Stage. P-021 C5 states only that its own prohibitions leave a manifest-derived source-stash retirement *permissible*; it never mandates one, so this workspace keeps retirement handoff-only and Ship does not call `backlogit_stash_archive`:
