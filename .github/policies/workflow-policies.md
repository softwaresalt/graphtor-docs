...
**Version**: 1.13.0 | **Generated by**: autoharness
...
**Applies when**: The `backlogit` capability pack is installed and the registry advertises `features.shipments: true`.

**Scope**: Ensures archive files are not silently lost after the `shipment-reconcile` safe-close archival sequence runs during Ship Step 6 post-merge closure in backlogit workspaces.

**Precondition**: The `shipment-reconcile` safe-close procedure has archived the shipment's manifest items and the shipment record as single artifacts, each carrying its delivered-work commit evidence. (The cascade `backlogit_ship_shipment` is forbidden by P-015 and is never the precondition here.)

**Gate Point**: Ship Step 6, immediately after safe-close completes and before the backlog state is committed.
...
**Violation Action (halt first; recovery is approval-gated and exact-path only)**:

1. **Halt.** Do not run any recovery command yet. Restoring `.backlogit/` artifacts is a Git mutation with `ActionRisk: destructive` and `change_kind: rollback`, which Constitution VII (Destructive Command Approval) requires an operator to approve first.
2. Record a `ProposedAction` naming the **exact** deleted archive paths to restore — or the **exact** revert commit when the loss was already committed — plus the `rollback` field and `approval_required: true`. Set `ActionResult: blocked`.
3. Request **explicit real-time operator approval** for that exact recovery and broadcast a P-005 violation event. Without approval the agent stays halted with `ActionResult: blocked` and does NOT commit the corrupt backlog state.
4. **Only after** approval, execute exactly the approved recovery: `git restore -- {exact deleted archive paths}`, or `git revert {exact commit}`. Never a broad directory restore such as restoring all of `.backlogit/archive/`, never unrelated paths or unrelated history, never `git reset`, never a force operation, and never a silent or automatic recovery. Set `ActionResult: applied`.
5. Re-verify with `git status -- ".backlogit/archive/"` — confirm no deletions remain — then stage the restored files: `git add .backlogit/archive/`
6. Proceed to the commit step only after verification passes. If restoration fails, or approval is unavailable, withheld, or denied, remain halted and prompt the operator.
7. Operator approval addresses a command's destructiveness. It never grants role authority an agent's Role Boundary withholds, so an approved restore never licenses a role-forbidden backlog deletion (P-010).
...
- Claim, close, ship, or archive a shipment manifest on behalf of Ship — shipment closure and shipment archival are Ship-only
- Record commit evidence on any item
- Delete backlog items or stash entries. `stash_remove` is **always forbidden** for Stage: operator approval addresses a command's destructiveness but never grants the role authority P-010 withholds, so no approval makes it legal. Use `stash_archive`, or hand the deletion to the operator or a separately authorized recovery path

**Stage MAY** (within its legitimate scope):
- Create and update backlog items, item sections, deliberation artifacts, and planning artifacts
- Add and remove dependency edges and typed artifact links between Stage-owned items
- Apply Stage-ratified status normalization to Stage-owned work items
- Archive Stage-owned backlog work items superseded or withdrawn before shipment, and retire consumed stash entries through `stash_archive`
- Create, update, and publish **Stage-owned queued shipment manifests** in both Mode H (fresh harvest) and Mode R (ratified handoff), including the atomic Mode R create — Ship then claims, closes, and archives them
...
**Ship MUST NOT**:
- Create backlog items, create shipments, add shipment members, or update item planning fields (scope, acceptance criteria, dependencies, priority, title, description, labels, sections)
- Issue any `backlogit_update_item` call carrying a field other than `commit`
- Delete a mistakenly created or mismatched shipment or backlog item — deletion is an operator decision, never a Ship cleanup
- Perform stash operations, triage, or deliberation
- Create or modify deliberation, spike, plan, or review artifacts
- Commit or push directly to `main`
- Call the cascade `backlogit_ship_shipment` (P-015), or transition a shipment record to `done` — `done` is not in the shipment status enum

**Ship MAY** (within its legitimate scope):
- Claim the Stage-prepared shipment that scopes the session, and move its members through `active` / `done`
- Close **that one** shipment by transitioning its record to the terminal shipment status `shipped` and archiving that record, plus each manifest member, as single artifacts through `shipment-reconcile` safe-close
- Return a non-terminal member of the current shipment through the status-preserving `return-blocked` operation, recording only the exact blocked reason that operation requires
- Write the delivered-work merge commit into the current shipment's or a current manifest member's frontmatter `commit` with a **commit-only** `update_item` (CLI `backlogit update {id} --commit {sha}`) before any terminal mutation, optionally adding a **supplemental** `track_commit` commit-link that never substitutes for it
- Create, update, and resolve Ship-owned checkpoints from the current or a prior session in the same shipment or PR scope, after validating owner and scope
- Rebuild derived state (`sync_index`) and acknowledge Ship-addressed hook events
...
**Violation Action**: Record a P-010 violation (via P-005 telemetry) and halt. Do not proceed past the boundary even if the operator requests work outside scope — redirect to the correct agent instead. Operator approval addresses a command's destructiveness, never its role authority: an approved destructive command is still a P-010 violation when the acting agent's Role Boundary forbids that category of mutation.
...
**Applies when**: The `backlogit` capability pack is installed, the registry advertises `features.shipments: true`, and shipments may cover a partial subset of a covering feature's tasks (partial-feature shipments).

**Statement**: The ship agent MUST close a shipment by archiving **only** the shipment manifest's explicit item IDs — read from the shipment record's `custom_fields.items` — one artifact at a time, via the `shipment-reconcile` safe-close procedure, and finally the shipment record itself as its own single artifact. It MUST NOT call the cascade `backlogit_ship_shipment` for closure. The cascade op treats a shipment as a proxy for its covering feature and archives the parent feature and any unshipped sibling tasks — corrupting the backlog whenever the shipment is a partial-feature shipment that intentionally excludes them.

**Selected-path closure contract (NON-NEGOTIABLE)**: For each artifact, safe-close dispatches **once**, before any mutation, on whether the **selected invocation transport** for the archive operation itself accepts the delivered-work SHA **and** guarantees, in its own tool contract, that the value is persisted to the archived artifact's frontmatter `commit` atomically with archival. Registry parameter presence alone is **not** that guarantee: the registry declares `archive_item.params.commit_sha`, but the installed CLI mapping `backlogit archive {id}` has no commit flag, so a CLI invocation always takes the non-atomic path. The two paths are **mutually exclusive** and each artifact is archived **exactly once**:

- **Atomic path** — one archive call carrying the SHA; **no** later move or archive runs on that artifact at all.
- **Live frontmatter-commit path** — write the SHA to the still-live artifact's frontmatter `commit` with a **commit-only** update (MCP `update_item` carrying only `commit`, or CLI `backlogit update {id} --commit {sha}`), verify it, and only then run the non-atomic terminal/archive sequence exactly once: the task terminal status `done` for a member task, the shipment terminal status `shipped` for the shipment record (`done` is not in the shipment status enum). Never write evidence after a terminal transition that relocates the artifact.

`track_commit` / commit_links are **supplemental** provenance and never substitute for the canonical frontmatter `commit`. If neither path is available, safe-close halts before any terminal mutation. Ship and this policy delegate the sequence to the skill's selected path; neither prescribes an unconditional move-to-done-then-archive.

**Logical shipment lock**: The lock file created from the original queue path, `.backlogit/queue/.{shipment_id}.md.lock`, is the canonical **logical** shipment lock for the whole pre-mode → relocation → safe-close → post-mode sequence. It remains held across the relocation this procedure performs, every conforming safe-close writer MUST honor that original lock identity, and release uses the **original queue path** and tolerates a missing or moved target. The lock file does not follow the artifact, so it provides no same-directory physical protection over the relocated copy; a second acquisition attempted on the now-missing queue target fails closed, which is the intended single-writer guarantee. Ambiguous target or lock state halts closure.

**Precondition**: The shipment manifest (`custom_fields.items`) has been read, and the **protected set** — the covering feature plus every unshipped sibling task that is not in the manifest — has been computed from **expected IDs** (partial-feature detection scanning both queue and archive). A clean git baseline for `.backlogit/` has been captured, and a **baseline integrity gate** has confirmed every protected-set member is present in `.backlogit/queue/` before any manifest item is archived. A protected-set member already archived or missing at baseline is treated as a pre-existing cascade and halts closure.

**Required Check (verify-after-each invariant)**: After archiving each single manifest item, confirm every protected-set member still exists in `.backlogit/queue/` — not moved to `.backlogit/archive/`, not deleted from the working tree. Manifest items legitimately archived before this run (`pre-archived`) are excluded from the item loop to avoid false positives; this exemption applies to **manifest items only** — the protected set has **no** pre-archived exemption, so any protected-set member found archived or missing is a cascade.

**Postcondition**: Every shipment manifest item **and** the shipment record are archived in `.backlogit/archive/` exactly once, each carrying its frontmatter `commit` evidence, the entire protected set remains present in `.backlogit/queue/`, and the cascade `backlogit_ship_shipment` was never called.

**Violation Action (halt first; recovery is approval-gated and exact-path only)**:

1. If any non-manifest artifact (the parent feature or a sibling task) was archived or deleted, **halt** with `HALT — cascade detected, revert required`. Do not run any recovery command yet: restoring or reverting `.backlogit/` artifacts is `ActionRisk: destructive` with `change_kind: rollback`, which Constitution VII requires an operator to approve first.
2. Record a `ProposedAction` naming the **exact** identified protected-set paths to restore, or the **exact** revert commit that introduced the cascade, plus the `rollback` field and `approval_required: true`. Set `ActionResult: blocked`. Broadcast a P-005 violation event naming the cascaded artifact IDs and request **explicit real-time operator approval**.
3. **Only after** approval, execute exactly the approved recovery: `git restore -- {exact identified protected-set paths}`, or `git revert {exact cascade commit}`. Never a broad directory restore, never unrelated paths or unrelated history, never `git reset`, never a force operation, and never a silent or automatic recovery. Set `ActionResult: applied`.
4. Re-verify that the full protected set is intact, record the verification, and remain halted. Recovery restores the protected set; it does not resume closure. Do NOT commit a corrupt backlog and do NOT auto-prune the manifest. If approval is unavailable, withheld, or denied, remain halted with `ActionResult: blocked`.
5. Operator approval addresses destructiveness only. It never grants role authority a Role Boundary withholds, so an approved recovery never licenses a role-forbidden backlog deletion (P-010).
...
* **C5 — Ship capture-only carve-out** (hardening H2): Ship MAY create stash entries for capture only, and MUST NOT triage, prioritize, re-classify, edit, harvest, deliberate on, DISCRETIONARILY remove, or DISCRETIONARILY archive them — while the manifest-derived retirement of the source stash entry that fed the shipped scope, after a successful merge (the `backlogit_stash_archive` on `custom_fields.source_stash_id` contemplated by Ship's post-merge Step 7), is NOT PROHIBITED BY THIS CLAUSE. That is a scoping statement, not a grant: C5 says only that its own prohibitions do not reach that operation, and it never mandates it. Whether Ship may actually perform it is determined solely by P-010's Role Boundary — which in this workspace FORBIDS it, so Ship does not call `backlogit_stash_archive` and records the retirement as a Stage handoff instead. See the **Workspace resolution of the C5 scoping clause** below for the single authority model. Removal and archival are named as SEPARATE prohibited discretionary dispositions: archival is a distinct backlogit operation that also takes a Stage-owned deferred entry out of Stage's triage queue, so a prohibition naming only removal would leave the same loss reachable by another verb. The DISCRETIONARY qualifier governs BOTH verbs, and C5's silence on the manifest-derived post-merge retirement is unchanged — silence that P-010 then resolves as forbidden here. This capture-only creation grant and its accompanying prohibitions govern ONLY the P-021 C2 deferred-scope-expansion stash entries this policy introduces; they do not narrow, restrict, or otherwise apply to any separate, pre-existing stash-entry creation allowances a Ship agent may hold (for example, the Ship agent's pre-merge Step 10 / post-merge Step 6 follow-up-stash steps), which remain governed solely by P-010's Role Boundary and are unaffected by this clause — and which, in this workspace, P-010 likewise resolves as handoff-only.
...
**Relationship to P-010**: The C5 Ship capture-only carve-out narrows, rather than conflicts with, P-010's Ship Role Boundary. Ship's Forbidden column already prohibits stash triage, prioritization, and deliberation; P-021 adds a single narrow Allowed-column exception — creating a capture-only stash entry — while every other stash verb named in P-010's Forbidden column remains forbidden to Ship. In this workspace that Forbidden column names stash removal and archival **including the post-merge Step 7 source-artifact retirement**, so P-010's prohibition is not limited to the discretionary dispositions C5 enumerates: it also covers the manifest-derived retirement C5 leaves unaddressed. P-010 is the authority on what Ship may do; C5 only bounds what P-021 itself prohibits.

**Workspace resolution of the C5 scoping clause**: C5's "pre-existing stash-entry creation allowances" sentence is a *scoping* clause — it prevents C5's prohibitions from narrowing allowances Ship already holds — and its parentheticals (the follow-up-stash steps and the post-merge source-stash retirement) are illustrative, not grants. In this workspace Ship holds no such allowances: `_ship.agent.md` pre-merge Step 10, post-merge Step 6, and post-merge Step 7 are all **handoff-only**, recording follow-ups and source-artifact retirement for a future Stage session rather than mutating stash state. The clause is therefore vacuous here rather than violated, and the single authority model across the Role Boundary, the mutation-classification table, and the procedures is: **Ship creates a stash entry only for a C2 deferred-scope-expansion capture, and touches stash state in no other way.** Concretely, `backlogit_stash_archive` and `backlogit_stash_remove` are forbidden to Ship in every context here, including the post-merge Step 7 source-artifact retirement. That resolution is consistent, not fail-closed guesswork: because Ship's procedures never attempt the operation in the first place, P-010 yields a Stage handoff rather than an unauthorized mutation, and there is no halt to incur. Because C2 makes the capture a precondition and C7 halts without it, this one grant is what keeps P-021 satisfiable for Ship.
